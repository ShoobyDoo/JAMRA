...
export function HotZoneIndicator({ zone }: HotZoneIndicatorProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute ${config.iconPosition} ${isHorizontal ? "top-1/2 -translate-y-1/2" : "left-1/2 -translate-x-1/2"}
      ...
      >
        ...
        {/* Helper text */}
        <div
          className={`absolute ${isHorizontal ? "top-full mt-2" : "left-full ml-2"} whitespace-nowrap`}
        >
          <div className="bg-black/75 backdrop-blur-md px-2.5 py-1 rounded-md text-white text-xs font-medium shadow-xl border border-white/10">
            Click to{" "}
            {zone === "left" || zone === "top" ? "go back" : "go forward"}
          </div>
        </div>
      </div>
    </div>
