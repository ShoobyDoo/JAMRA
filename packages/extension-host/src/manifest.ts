...
export function validateManifest(manifest: ExtensionManifest): void {
  ...
  if (!manifest.version || typeof manifest.version !== "string") {
    ...
    issues.push(
      "`version` must follow semver (e.g. 1.2.3). Use prerelease/build metadata if needed.",
    );
  }
  ...
  if (
    !Array.isArray(manifest.languageCodes) ||
  ...
  if (!manifest.capabilities || manifest.capabilities.catalogue !== true) {
    issues.push(
      "`capabilities.catalogue` must be true (all extensions must support catalogue).",
    );
  }

  if (issues.length > 0) {
    throw new ManifestValidationError(
      `Invalid manifest for extension ${manifest.id ?? "<unknown>"}`,
      issues,
    );
  }
}
