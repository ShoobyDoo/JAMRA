...
function validateChecksum(basePath, value, errors) {
    if (!isRecord(value)) {
...
function validateSignature(basePath, value, errors) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value)) {
    ...
    const algorithm = value.algorithm;
    if (algorithm !== "pgp" && algorithm !== "minisign" && algorithm !== "rsa" && algorithm !== "ed25519") {
        errors.push(`${basePath}.algorithm must be pgp, minisign, rsa, or ed25519.`);
        return undefined;
    }
    if (!isNonEmptyString(value.value)) {
        errors.push(`${basePath}.value is required.`);
...
function validatePublisher(basePath, value, errors) {
    if (!isRecord(value)) {
        errors.push(`${basePath} must be an object.`);
        return undefined;
    }
    ...
    if (value.contact !== undefined) {
        if (!isNonEmptyString(value.contact)) {
            errors.push(`${basePath}.contact must be a non-empty string when provided.`);
        }
        else {
...
function validateVersion(basePath, value, errors) {
    if (!isRecord(value)) {
        errors.push(`${basePath} must be an object.`);
        return undefined;
    }
    ...
    const checksum = validateChecksum(`${basePath}.checksum`, value.checksum, errors);
    ...
    if (value.publishedAt !== undefined) {
        if (!isNonEmptyString(value.publishedAt)) {
            errors.push(`${basePath}.publishedAt must be a non-empty string when provided.`);
        }
        else {
    ...
    if (value.minHostVersion !== undefined) {
        if (!isNonEmptyString(value.minHostVersion) || !valid(value.minHostVersion)) {
            errors.push(`${basePath}.minHostVersion must be a semantic version when provided.`);
        }
        else {
            // assigned later once version is confirmed
        }
    ...
    if (value.minSdkVersion !== undefined) {
        if (!isNonEmptyString(value.minSdkVersion) || !valid(value.minSdkVersion)) {
            errors.push(`${basePath}.minSdkVersion must be a semantic version when provided.`);
        }
        else {
            // assigned later once version is confirmed
        }
    ...
    if (value.compatibilityNotes !== undefined) {
        if (!isNonEmptyString(value.compatibilityNotes)) {
            errors.push(`${basePath}.compatibilityNotes must be a non-empty string when provided.`);
        }
        else {
            // assigned later once version is confirmed
        }
    }
    const signature = validateSignature(`${basePath}.signature`, value.signature, errors);
...
function validateExtension(basePath, value, errors) {
    if (!isRecord(value)) {
        errors.push(`${basePath} must be an object.`);
        return undefined;
    }
    ...
    const summary = isNonEmptyString(value.summary) ? value.summary.trim() : undefined;
    if (!summary) {
    ...
    if (value.description !== undefined) {
        if (!isNonEmptyString(value.description)) {
            errors.push(`${basePath}.description must be a non-empty string when provided.`);
        }
        else {
    ...
    if (value.homepage !== undefined) {
        if (!isNonEmptyString(value.homepage)) {
            errors.push(`${basePath}.homepage must be a non-empty string when provided.`);
        }
        else {
    ...
    if (value.repository !== undefined) {
        if (!isNonEmptyString(value.repository)) {
            errors.push(`${basePath}.repository must be a non-empty string when provided.`);
        }
        else {
    ...
    if (value.tags !== undefined) {
        if (!Array.isArray(value.tags) || value.tags.some((tag) => !isNonEmptyString(tag))) {
            errors.push(`${basePath}.tags must be an array of non-empty strings when provided.`);
        }
        else {
            extension.tags = value.tags.map((tag) => tag.trim());
        }
    }
    if (value.categories !== undefined) {
        if (!Array.isArray(value.categories) || value.categories.some((category) => !isNonEmptyString(category))) {
            errors.push(`${basePath}.categories must be an array of non-empty strings when provided.`);
        }
        else {
            extension.categories = value.categories.map((category) => category.trim());
        }
    }
    if (value.license !== undefined) {
        if (!isNonEmptyString(value.license)) {
            errors.push(`${basePath}.license must be a non-empty string when provided.`);
        }
        else {
    ...
    if (maintainersValue !== undefined) {
        if (!Array.isArray(maintainersValue)) {
            ...
            maintainersValue.forEach((entry, index) => {
                const publisher = validatePublisher(`${basePath}.maintainers[${index}]`, entry, errors);
                if (publisher)
    ...
    value.versions.forEach((entry, index) => {
        const version = validateVersion(`${basePath}.versions[${index}]`, entry, errors);
        if (version)
            versions.push(version);
    });
    if (versions.length === 0) {
        errors.push(`${basePath}.versions must contain at least one valid release.`);
        return undefined;
    }
...
function validateManifestMetadata(value, errors) {
    if (!isRecord(value)) {
    ...
    if (value.description !== undefined) {
        if (!isNonEmptyString(value.description)) {
            errors.push("manifest.registry.description must be a non-empty string when provided.");
        }
        else {
    ...
    if (value.homepage !== undefined) {
        if (!isNonEmptyString(value.homepage)) {
            errors.push("manifest.registry.homepage must be a non-empty string when provided.");
        }
        else {
    ...
    if (value.supportUrl !== undefined) {
        if (!isNonEmptyString(value.supportUrl)) {
            errors.push("manifest.registry.supportUrl must be a non-empty string when provided.");
        }
        else {
    ...
    if (value.icon !== undefined) {
        if (!isNonEmptyString(value.icon)) {
            errors.push("manifest.registry.icon must be a non-empty string when provided.");
        }
        else {
    ...
    if (maintainers !== undefined) {
        if (!Array.isArray(maintainers)) {
            errors.push("manifest.registry.maintainers must be an array when provided.");
        }
        else {
            const parsed = [];
            maintainers.forEach((entry, index) => {
                const publisher = validatePublisher(`manifest.registry.maintainers[${index}]`, entry, errors);
                if (publisher)
...
export function validateRegistryManifest(input) {
    ...
    if (Array.isArray(input.extensions)) {
        input.extensions.forEach((entry, index) => {
            const extension = validateExtension(`manifest.extensions[${index}]`, entry, errors);
            if (extension)
    ...
    if (input.generatedAt !== undefined) {
        if (!isNonEmptyString(input.generatedAt)) {
            errors.push("manifest.generatedAt must be a non-empty string when provided.");
        }
    }
...
export function getLatestVersion(extension, options = {}) {
...
export function findVersion(extension, version) {
    return extension.versions.find((candidate) => candidate.version === version);
}
export function summarizeReleaseNotes(notes, limit = 280) {
