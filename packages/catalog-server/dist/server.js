...
function resolveExtensionPath(customPath) {
    if (customPath) {
        return path.resolve(customPath);
    }
    return path.resolve(process.cwd(), "packages/example-extension/dist/index.js");
}
...
function coerceSettings(payload) {
...
function resolveExtensionErrorStatus(error) {
    if (!(error instanceof Error))
        return 500;
    const message = error.message.toLowerCase();
    if (message.includes("persistent extension storage"))
        return 503;
...
function parseRegistryEnv(value) {
    return value
        .split(",")
        ...
        const parts = entry
            ...
            .filter(Boolean);
        if (parts.length === 0) {
            throw new Error(`Invalid registry entry at position ${index + 1}: ${entry}`);
        }
        let id;
        let url;
        let label;
...
function parseRegistryJson(json) {
    try {
        ...
        throw new Error(`JAMRA_EXTENSION_REGISTRIES_JSON is invalid JSON: ${String(error)}`);
    }
}
function resolveRegistrySources() {
    ...
    const officialManifestPath = fileURLToPath(new URL("./extensions/registries/official.json", import.meta.url));
    return [
        {
            id: "jamra-official",
...
function buildSourceMetadata(resolved) {
    return {
...
function buildUpdateDetails(resolved) {
    return {
        version: resolved.version.version,
        downloadUrl: resolved.version.downloadUrl,
        checksum: resolved.version.checksum.value,
...
export async function startCatalogServer(options = {}) {
    ...
    let bootstrapExtensionPath;
    try {
        bootstrapExtensionPath = resolveExtensionPath(options.extensionPath ?? process.env.JAMRA_EXTENSION_PATH);
    }
    catch (error) {
    ...
    if (process.env.JAMRA_DISABLE_SQLITE !== "1") {
        try {
            ...
            console.warn("Failed to initialize SQLite catalog store. Falling back to in-memory cache.");
    ...
    if (repository && storage) {
        ...
        const bootstrapExists = Boolean(bootstrapExtensionPath && fs.existsSync(bootstrapExtensionPath));
        if (bootstrapExists) {
            try {
                const bootstrapModule = await importExtension(bootstrapExtensionPath);
                const bootstrapId = bootstrapModule.manifest.id;
                const existing = installed.find((extension) => extension.id === bootstrapId);
                if (!existing ||
                    !existing.entryPath ||
                    ...
                    try {
                        ...
                        console.warn(`Failed to install bootstrap extension from ${bootstrapExtensionPath}.`, error);
                    }
                }
                if (explicitId && bootstrapId !== explicitId) {
                    console.warn(`Bootstrap extension id ${bootstrapId} did not match expected ${explicitId}.`);
                }
            }
            catch (error) {
                console.warn(`Unable to read bootstrap extension at ${bootstrapExtensionPath}.`, error);
            }
        }
        else if (bootstrapExtensionPath) {
            console.warn(`Bootstrap extension not found at ${bootstrapExtensionPath}.`);
        }
        if (installed.length === 0 && bootstrapExists) {
            try {
                const installedExtension = await extensionManager.installFromFile(bootstrapExtensionPath, {
                    enabled: true,
                });
                installed = extensionManager.listExtensions();
                if (explicitId && installedExtension.id !== explicitId) {
                    console.warn(`Bootstrap extension id ${installedExtension.id} did not match expected ${explicitId}.`);
                }
            }
            catch (error) {
                console.warn(`Failed to bootstrap extension from ${bootstrapExtensionPath}.`, error);
            }
        }
        if (explicitId) {
            try {
    ...
    const ensureExtensionLoaded = (req, res) => {
    ...
    app.post("/api/extensions", async (req, res) => {
        ...
        if (source && typeof source === "object") {
            const registryId = typeof source.registryId === "string" ? source.registryId.trim() : "";
            const extensionId = typeof source.extensionId === "string" ? source.extensionId.trim() : "";
            const version = typeof source.version === "string" ? source.version.trim() : undefined;
            if (!registryId || !extensionId) {
                res.status(400).json({
                    error: "source.registryId and source.extensionId are required to install from a registry.",
                });
                return;
            }
            let resolved;
            try {
                resolved = await registryService.resolveExtension(registryId, extensionId, { version });
            }
            catch (error) {
            ...
            try {
                const extension = await extensionManager.installFromFile(asset.filePath, installOptions);
                ...
                handleError(res, error, "Failed to install extension", resolveExtensionErrorStatus(error));
            }
            finally {
                await asset.cleanup();
            }
        ...
        try {
            const extension = await extensionManager.installFromFile(filePath, installOptions);
            if (extension.enabled && extension.loaded) {
                activeExtensionId = extension.id;
            }
            else if (!activeExtensionId) {
    ...
    app.post("/api/extensions/:id/enable", async (req, res) => {
        ...
        try {
            const extension = await extensionManager.enableExtension(id, parsedSettings);
            activeExtensionId = extension.id;
            res.json({ extension });
        }
        catch (error) {
            handleError(res, error, "Failed to enable extension", resolveExtensionErrorStatus(error));
        }
    });
    app.post("/api/extensions/:id/disable", async (req, res) => {
        ...
        try {
            ...
            handleError(res, error, "Failed to disable extension", resolveExtensionErrorStatus(error));
        }
    });
    app.post("/api/extensions/:id/check-updates", async (req, res) => {
        ...
        try {
            ...
            if (!registryId) {
                res.status(400).json({
                    error: "Extension is not linked to a registry and cannot participate in update checks.",
                });
                return;
            }
            const resolved = await registryService.resolveExtension(registryId, extension.id);
            if (!resolved) {
                res.status(404).json({
            ...
            else if (extension.updateState?.acknowledgedVersion === updateDetails.version &&
    ...
    app.post("/api/extensions/:id/acknowledge-update", async (req, res) => {
        ...
        try {
            ...
            if (!extension.updateState?.latest ||
    ...
    app.patch("/api/extensions/:id/settings", async (req, res) => {
        ...
        try {
            const extension = await extensionManager.updateSettings(id, parsedSettings);
            res.json({ extension });
        }
        catch (error) {
            handleError(res, error, "Failed to update extension settings", resolveExtensionErrorStatus(error));
        }
    });
    app.delete("/api/extensions/:id", async (req, res) => {
        ...
        try {
            ...
            handleError(res, error, "Failed to uninstall extension", resolveExtensionErrorStatus(error));
        }
    });
    ...
    app.get("/api/manga/:id/chapters/:chapterId/pages", async (req, res) => {
        ...
        if (!extensionId)
            return;
        try {
            const result = await service.syncChapterPages(extensionId, req.params.id, req.params.chapterId);
    ...
    app.get("/api/filters", async (req, res) => {
        ...
        if (!extensionId)
            return;
        try {
            const filters = (await service.getFilters(extensionId));
