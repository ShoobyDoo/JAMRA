import type Database from "better-sqlite3";
import type { ExtensionCache } from "@jamra/extension-sdk";

interface CacheRow {
  value: string;
  expires_at: number | null;
}

function toStoredValue(value: unknown): string {
  return JSON.stringify({ v: value });
}

function fromStoredValue<T>(raw: string): T {
  return (JSON.parse(raw) as { v: T }).v;
}

export class SqliteExtensionCache implements ExtensionCache {
  private readonly getStmt;
  private readonly setStmt;
  private readonly deleteStmt;
  private readonly cleanupExpiredStmt;

  constructor(private readonly db: Database.Database) {
    this.getStmt = this.db.prepare(
      `SELECT value, expires_at FROM extension_cache WHERE namespace = @namespace AND cache_key = @cache_key`,
    );

    this.setStmt = this.db.prepare(
      `INSERT INTO extension_cache (namespace, cache_key, value, expires_at, updated_at)
       VALUES (@namespace, @cache_key, @value, @expires_at, @updated_at)
       ON CONFLICT(namespace, cache_key) DO UPDATE SET
         value = excluded.value,
         expires_at = excluded.expires_at,
         updated_at = excluded.updated_at`,
    );

    this.deleteStmt = this.db.prepare(
      `DELETE FROM extension_cache WHERE namespace = @namespace AND cache_key = @cache_key`,
    );

    this.cleanupExpiredStmt = this.db.prepare(
      `DELETE FROM extension_cache WHERE expires_at IS NOT NULL AND expires_at <= @now`,
    );
  }

  async get<T>(namespace: string, key: string): Promise<T | undefined> {
    const row = this.getStmt.get({ namespace, cache_key: key }) as
      | CacheRow
      | undefined;
    if (!row) return undefined;

    if (row.expires_at && row.expires_at <= Date.now()) {
      this.deleteStmt.run({ namespace, cache_key: key });
      return undefined;
    }

    return fromStoredValue<T>(row.value);
  }

  async set<T>(
    namespace: string,
    key: string,
    value: T,
    ttlMs?: number,
  ): Promise<void> {
    this.setStmt.run({
      namespace,
      cache_key: key,
      value: toStoredValue(value),
      expires_at: ttlMs ? Date.now() + ttlMs : null,
      updated_at: Date.now(),
    });
  }

  async delete(namespace: string, key: string): Promise<void> {
    this.deleteStmt.run({ namespace, cache_key: key });
  }

  async cleanupExpired(): Promise<number> {
    const result = this.cleanupExpiredStmt.run({ now: Date.now() });
    return result.changes ?? 0;
  }
}
