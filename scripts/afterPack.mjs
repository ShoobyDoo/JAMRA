...
function resolveBetterSqliteModule(nodeModulesDir) {
  ...
  if (fs.existsSync(pnpmDir)) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.join(pnpmDir, entry.name, 'node_modules', 'better-sqlite3');
...
export default async function afterPack(context) {
  ...
  let archLabel = archKey != null && !Number.isNaN(archKey) ? archLabels[archKey] : null;
  ...
  const platformKey = archLabel ? `${electronPlatformName}-${archLabel}` : electronPlatformName;
  ...
  if (!sqliteModulePath) {
    const projectModule = findProjectBetterSqliteModule(context.packager.projectDir);
    if (projectModule) {
      console.log(`[afterPack] better-sqlite3 missing for ${platformKey}, copying from project cache`);
  ...
  if (!sqliteModulePath) {
    console.warn(`[afterPack] better-sqlite3 not found in ${unpackedNodeModulesDir} for ${platformKey} and no fallback copy available`);
    return;
  }
